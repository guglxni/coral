...
/// Request-time source input context exposed to source input resolvers.
///
...
impl SourceInputContext {
    #[must_use]
    /// Builds request input context from one selected query source.
...
mod tests {
    ...
    use crate::{QuerySource, SourceInputContext};

    #[test]
    fn source_input_context_keeps_only_request_input_contract() {
        ...
        let context = SourceInputContext::from_query_source(&source);
